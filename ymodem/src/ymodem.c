...
/**
 * @brief 通过YMODEM协议接受串口发送过来的数据并存为文件
 * @param path: 文件保存路径
 */
YmodemStatus ymodem_receive(YmodemFileInfo* file_info, const TCHAR* path)
{
    char file_path[256];
        ...
        if (ymodem_receive_packet[0] == SOH || ymodem_receive_packet[0] == STX)
        {
            if (!head_flag && ymodem_receive_packet[1] == 0x00 && ymodem_receive_packet[2] == 0xFF)
            {
                ...
                for (uint8_t i = 3, j = 0; i < 128; j++)
                {
                    ...
                    {
                        strncpy(received_size, (char*)&ymodem_receive_packet[i + 1],
                                j - 4 - strlen(file_info->file_name));
